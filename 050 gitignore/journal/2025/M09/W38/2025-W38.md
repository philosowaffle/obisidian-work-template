...
[[2025-W37|<< W37]] | 2025-W38 | [[2025-W39|W39 >>]]
...
FROM #project
WHERE (created >= date(2025-09-15) and created <= date(2025-09-21)) or (archived >= date(2025-09-15) and archived <= date(2025-09-21)) or (project-status = "In Progress" or project-status = "Blocked"  or (project-status = "Standby"))
```
...
WHERE created >= date("2025-09-15") and created <= date("2025-09-21")
sort file.name ascending
```

tab: 🕐 Log
```dataviewjs
let weekStart = "2025-09-15";
let weekEnd = "2025-09-21";
let days = dv
	.pages()
	//.pages('"010 projects"')
	//.limit(3)
...
LIST
from #meeting 
where file.ctime >= date("2025-09-15") and file.ctime <= date("2025-09-21")
sort file.name ascending
```
````

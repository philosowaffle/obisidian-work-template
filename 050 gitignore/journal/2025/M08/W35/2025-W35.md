...
FROM #project
WHERE (created >= date(2025-08-31) and created <= date(2025-09-06)) or (archived >= date(2025-08-31) and archived <= date(2025-09-06)) or (project-status = "In Progress" or project-status = "Blocked"  or (project-status = "Standby"))
```
...
WHERE file.ctime >= date("2025-08-31") and file.ctime <= date("2025-09-06")
sort file.name ascending
```

tab: 🕐 Log
```dataviewjs
let weekStart = "2025-08-31";
let weekEnd = "2025-09-06";
let days = dv
	.pages()
	//.pages('"010 projects"')
	//.limit(3)
